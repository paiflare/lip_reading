...
class LipReadinNN(nn.Module):
    def __init__(self, tokens_to_id):
        ...
        self.transformer = nn.Transformer(d_model=512, nhead=1, num_encoder_layers=2, num_decoder_layers=2, 
                                          dim_feedforward=512)
    ...
    def forward(self, input_batch, target_list_of_tokens=None):      
        ...
        output = output.reshape(win_len, frames_num*batch_size, 512) # [time, batch, feature=512] for LSTM
        output, (h_n, c_n) = self.LSTM(output) # output shape is [time, batch, feature=1024]
        output = output[-1, :, :] # last state output in LSTM seq [batch, feature=1024]
        
        output = output.reshape(frames_num, batch_size, 1024) # [frames_num, batch, feature=1024] for Transformer
        ...
        if self.training:
            ...
            target_output = torch.moveaxis(target_output, (0,1), (1,0)) # [seq_len, batch, emb_size=512] for Transformer
        else:
            ...
            target_output = torch.moveaxis(target_output, (0,1), (1,0)) # [seq_len=1, batch, emb_size=512] for Transformer
            ...
            while i < max_iter:
                i += 1
                predict_output = self.transformer(output, target_output) # [seq_len=i, batch, emb_size=512]
        ...
        predict_output = self.transformer(output, target_output) # [seq_len, batch, emb_size=512]
